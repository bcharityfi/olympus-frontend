...
import { ReactComponent as OgvImg } from "../assets/tokens/token_OGV.svg";
import { ReactComponent as SOgvImg } from "../assets/tokens/token_sOGV.svg";

import { ogv_dai } from "./AllBonds";
import { JsonRpcSigner, StaticJsonRpcProvider } from "@ethersproject/providers";
import { IBaseAsyncThunk } from "src/slices/interfaces";
import { PairContract, RedeemHelper } from "../typechain";

export async function getMarketPrice({ networkID, provider }: IBaseAsyncThunk) {
  const ogv_dai_address = ogv_dai.getAddressForReserve(networkID);
  const pairContract = new ethers.Contract(ogv_dai_address || "", PairContractABI, provider) as PairContract;
 ...
 */
export async function getTokenPrice(tokenId = "olygive") {
  let resp;
  try {
...
function getSogvTokenImage() {
  return <SvgIcon component={SOgvImg} viewBox="0 0 100 100" style={{ height: "1rem", width: "1rem" }} />;
}

export function getOgvTokenImage(w?: number, h?: number) {
  const height = h == null ? "32px" : `${h}px`;
  const width = w == null ? "32px" : `${w}px`;
  return <SvgIcon component={OgvImg} viewBox="0 0 32 32" style={{ height, width }} />;
}

export function getTokenImage(name: string) {
  if (name === "ogv") return getOgvTokenImage();
  if (name === "sogv") return getSogvTokenImage();
}
