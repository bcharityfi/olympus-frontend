...
import { AwardAbi2, PrizePoolAbi, PrizePoolAbi2, SOGV } from "src/typechain";
...
export const changeApproval = createAsyncThunk(
  ...
  async ({ token, provider, address, networkID }: IChangeApprovalAsyncThunk, { dispatch }) => {
    ...
    const sogvContract = new ethers.Contract(addresses[networkID].SOGV_ADDRESS, ierc20Abi, signer) as SOGV;

    let approveTx;
    let depositAllowance = await sogvContract.allowance(address, addresses[networkID].PT_PRIZE_POOL_ADDRESS);
    ...
    depositAllowance = await sogvContract.allowance(address, addresses[networkID].PT_PRIZE_POOL_ADDRESS);
