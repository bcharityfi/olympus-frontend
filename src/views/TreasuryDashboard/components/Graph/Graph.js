...
export const ProtocolOwnedLiquidityGraph = () => {
  ...
  return (
    <Chart
      ...
      dataKey={["treasuryOgvDaiPOL"]}
      bulletpointColors={bulletpoints.pol}
      infoTooltipMessage={tooltipInfoMessages.pol}
      headerText="Protocol Owned Liquidity OGV-DAI"
      expandedGraphStrokeColor={theme.palette.graphStrokeColor}
      headerSubText={`${data && trim(data[0].treasuryOgvDaiPOL, 2)}% `}
      stopColor={[["rgba(128, 204, 131, 1)", "rgba(128, 204, 131, 0)"]]}
    />
  );
};

export const OGVStakedGraph = () => {
  const theme = useTheme();
  const { data } = useTreasuryMetrics({ refetchOnMount: false });
  ...
  const staked =
    ...
    data
      .map(metric => ({
        staked: (metric.sOgvCirculatingSupply / metric.ogvCirculatingSupply) * 100,
