...
export const PoolDeposit = props => {
  ...
  const poolAllowance = useSelector(state => {
    return state.account.pooling && state.account.pooling.sogvPool;
  });
  ...
  const setMax = () => {
    const value = parseFloat(sogvBalance);
    setQuantity(value);
  ...
  useEffect(() => {
    props.setInfoTooltipMessage([
      t`Deposit sOGV to win! Once deposited, you will receive a corresponding amount of 33T and be entered to win until your sOGV is withdrawn.`,
    ]);
  }, []);
          ...
          <Box display="flex" alignItems="center" flexDirection={`${isMobileScreen ? "column" : "row"}`}>
            ...
            {address && hasAllowance("sogv") ? (
              <Button
                ...
                style={{ margin: "5px" }}
              >
                {txnButtonText(pendingTransactions, "pool_deposit", t`Deposit sOGV`)}
              </Button>
            ) : (
              <Button
                className="pool-deposit-button"
          ...
          {newOdds > 0 && quantity > 0 && (
            <Box padding={1}>
              <Typography variant="body2" style={{ color: "#33BB33" }}>
                <Trans>
                  Depositing {quantity} sOGV will increase odds of winning to 1 in {newOdds}
                </Trans>
                &nbsp;
              </Typography>
            </Box>
            ...
            <div className="data-row">
              ...
              <Typography variant="body1" align="right">
                {isAppLoading ? (
                  ...
                  <>{new Intl.NumberFormat("en-US").format(sogvBalance)} sOGV</>
                )}
              </Typography>
            </div>
          </div>
