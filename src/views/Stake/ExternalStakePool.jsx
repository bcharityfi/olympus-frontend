...
import gOgvImage from "src/assets/tokens/gogv.png";
...
export default function ExternalStakePool() {
  ...
  const ogvLusdReserveBalance = useSelector(state => {
    return state.account && state.account.bonds?.ogv_lusd_lp?.balance;
  });
  ...
  return (
    <Zoom in={true}>
      <Paper className={`ogv-card secondary ${isSmallScreen && "mobile"}`}>
            ...
            <TableContainer className="stake-table">
              <Table>
                ...
                <TableBody>
                  <TableRow>
                    <TableCell>
                      <Box className="ogv-pairs">
                        {/* <BondLogo bond={{ bondIconSvg: OgvLusdImg, isLP: true }}></BondLogo> */}
                        <MultiLogo images={[gOgvImage, avaxImage]} avatarStyleOverride={avatarStyle} />
                        <Box width="16px" />
                        <Typography>gOGV-AVAX</Typography>
                      </Box>
                    </TableCell>
                    {/* <TableCell align="left">
                      {isLusdLoading ? (
                    ...
                    <TableCell align="left">
                      {isLusdLoading ? <Skeleton width="80px" /> : (trim(ogvLusdReserveBalance, 2) || 0) + " SLP"}
                    </TableCell> */}
            ...
            <div className="stake-pool">
              <div className={`pool-card-top-row ${isMobileScreen && "small"}`}>
                <Box className="ogv-pairs">
                  <MultiLogo images={[gOgvImage, avaxImage]} avatarStyleOverride={avatarStyle} />
                  <Box width="16px" />
                  <Typography gutterBottom={false}>gOGV-AVAX</Typography>
                </Box>
              </div>
